...
<script lang="ts" setup>
  ...
  import { RoleParams } from '@/api/sys-manage/role';
  ...
  const searchStore = storeToRefs(roleSearchStore);

  const formModel = ref<Partial<RoleParams>>(
    searchStore as Partial<RoleParams>
  );
  const search = async () => {
    ...
    roleSearchStore.$reset();
    roleSearchStore.$patch(searchParams);
    props.fetchData();
  };
  const reset = () => {
    roleSearchStore.$reset();
